...
// Hash: aabf6d6b368aca7f0a46835a8a8e2eb33d9573afa6d2e2fb6a9262bb4b270ea9
// Version: 0.1.3
package types
...
func (d *Deposit) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	if size := len(d.Pubkey); size > 48 {
		err = ssz.ErrBytesLengthFn("Deposit.Pubkey", size, 48)
		return
	}
	...
	if size := len(d.Signature); size > 96 {
		err = ssz.ErrBytesLengthFn("Deposit.Signature", size, 96)
		return
	}
...
func (v *ValidatorSet) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	if size := len(v.ValidatorIndices); size != 100 {
		err = ssz.ErrVectorLengthFn("ValidatorSet.ValidatorIndices", size, 100)
		return
	}
	for ii := 0; ii < 100; ii++ {
		dst = ssz.MarshalUint64(dst, v.ValidatorIndices[ii])
	}

	// Field (1) 'ValidatorPowers'
	if size := len(v.ValidatorPowers); size != 100 {
		err = ssz.ErrVectorLengthFn("ValidatorSet.ValidatorPowers", size, 100)
		return
	}
	for ii := 0; ii < 100; ii++ {
		dst = ssz.MarshalUint64(dst, v.ValidatorPowers[ii])
	}

	// Offset (2) 'ValidatorPubKeys'
	dst = ssz.WriteOffset(dst, offset)
	offset += len(v.ValidatorPubKeys) * 48

	// Field (2) 'ValidatorPubKeys'
	if size := len(v.ValidatorPubKeys); size > 100 {
		err = ssz.ErrListTooBigFn("ValidatorSet.ValidatorPubKeys", size, 100)
		return
	}
	for ii := 0; ii < len(v.ValidatorPubKeys); ii++ {
		dst = append(dst, v.ValidatorPubKeys[ii][:]...)
	}
...
func (v *ValidatorSet) UnmarshalSSZ(buf []byte) error {
	...
	// Field (0) 'ValidatorIndices'
	v.ValidatorIndices = ssz.ExtendUint64(v.ValidatorIndices, 100)
	for ii := 0; ii < 100; ii++ {
		v.ValidatorIndices[ii] = ssz.UnmarshallUint64(buf[0:800][ii*8 : (ii+1)*8])
	}

	// Field (1) 'ValidatorPowers'
	v.ValidatorPowers = ssz.ExtendUint64(v.ValidatorPowers, 100)
	for ii := 0; ii < 100; ii++ {
		v.ValidatorPowers[ii] = ssz.UnmarshallUint64(buf[800:1600][ii*8 : (ii+1)*8])
	}

	// Offset (2) 'ValidatorPubKeys'
	if o2 = ssz.ReadOffset(buf[1600:1604]); o2 > size {
		return ssz.ErrOffset
	}
	...
	{
		buf = tail[o2:]
		num, err := ssz.DivideInt2(len(buf), 48, 100)
		if err != nil {
			return err
		}
		v.ValidatorPubKeys = make([][48]byte, num)
		for ii := 0; ii < num; ii++ {
			copy(v.ValidatorPubKeys[ii][:], buf[ii*48:(ii+1)*48])
		}
	}
	return err
}
...
func (v *ValidatorSet) SizeSSZ() (size int) {
	...
	// Field (2) 'ValidatorPubKeys'
	size += len(v.ValidatorPubKeys) * 48
...
func (v *ValidatorSet) HashTreeRootWith(hh ssz.HashWalker) (err error) {
	...
	// Field (0) 'ValidatorIndices'
	{
		...
		subIndx := hh.Index()
		for _, i := range v.ValidatorIndices {
			hh.AppendUint64(i)
		}
		hh.Merkleize(subIndx)
	}
	...
	// Field (1) 'ValidatorPowers'
	{
		...
		subIndx := hh.Index()
		for _, i := range v.ValidatorPowers {
			hh.AppendUint64(i)
		}
		hh.Merkleize(subIndx)
	}
	...
	// Field (2) 'ValidatorPubKeys'
	{
		...
		subIndx := hh.Index()
		for _, i := range v.ValidatorPubKeys {
			hh.PutBytes(i[:])
		}
		numItems := uint64(len(v.ValidatorPubKeys))
