...
// Hash: 931179f77f8ca222a3a5328eb3115260593dc8c64ccbbd80c88af270b2d980d4
// Version: 0.1.3
package types
...
func (b *BeaconBlockBodyDeneb) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	if size := len(b.Redirects); size > 16 {
		err = ssz.ErrListTooBigFn("BeaconBlockBodyDeneb.Redirects", size, 16)
		return
	}
	{
...
func (r *Redirect) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	if size := len(r.Pubkey); size > 48 {
		err = ssz.ErrBytesLengthFn("Redirect.Pubkey", size, 48)
		return
	}
	...
	if size := len(r.NewPubkey); size > 48 {
		err = ssz.ErrBytesLengthFn("Redirect.NewPubkey", size, 48)
		return
	}
	...
	if size := len(r.Signature); size > 96 {
		err = ssz.ErrBytesLengthFn("Redirect.Signature", size, 96)
		return
	}
...
func (r *Redirect) HashTreeRootWith(hh ssz.HashWalker) (err error) {
	indx := hh.Index()
...
// GetTree ssz hashes the Redirect object
func (r *Redirect) GetTree() (*ssz.Node, error) {
