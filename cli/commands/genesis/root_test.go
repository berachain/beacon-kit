...
func TestOracle(t *testing.T) {
	...
	deposits := genesis.Deposits{
		{
			Pubkey:      bytes.B48{0xff, 0xff, 0xff, 0xff},
			Credentials: bytes.B32{0xaa, 0xaa, 0xaa},
			Amount:      math.U64(2025),
			Signature:   string("aabbcc"),
			Index:       int(0),
		},
		{
			Pubkey:      bytes.B48{0xee, 0xee, 0xee, 0xee},
			Credentials: bytes.B32{0xbb, 0xbb, 0xbb},
			Amount:      math.U64(5052),
			Signature:   string("ddeeff"),
			Index:       int(1),
		},
	}
	expectedRoot := common.Root{
	...
	require.Equal(t, expectedRoot, cliValRoot)
}
