...
func generateAttestationData() *deneb.AttestationData {
	return &deneb.AttestationData{
		Slot:  12345,
		Index: 67890,
...
func TestAttestationData_MarshalSSZ_UnmarshalSSZ(t *testing.T) {
	...
	testCases := []struct {
		name     string
		data     *deneb.AttestationData
		expected *deneb.AttestationData
		err      error
	}{
		{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			var unmarshalled deneb.AttestationData
