...
func generateValidDeposit() *deneb.Deposit {
	var pubKey crypto.BLSPubkey
	var signature crypto.BLSSignature
	var credentials deneb.WithdrawalCredentials
	amount := math.Gwei(32)
	index := uint64(1)

	return &deneb.Deposit{
		Pubkey:      pubKey,
...
func TestDeposit_MarshalUnmarshalSSZ(t *testing.T) {
	...
	var unmarshalledDeposit deneb.Deposit
