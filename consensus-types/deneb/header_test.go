...
func TestBeaconBlockHeader_Equals(t *testing.T) {
	...
	var (
		...
		lhs = deneb.NewBeaconBlockHeader(
	...
	tests := []struct {
		name string
		rhs  *deneb.BeaconBlockHeader
		want bool
	}{
		{
			name: "equal",
			rhs: deneb.NewBeaconBlockHeader(
				slot, valIdx, parentBlockRoot, stateRoot, bodyRoot,
			),
			want: true,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			rhsCopy := &deneb.BeaconBlockHeader{}
			*rhsCopy = *tt.rhs
...
func TestBeaconBlockHeader_Serialization(t *testing.T) {
	t.Parallel()
	original := deneb.NewBeaconBlockHeader(
		math.Slot(100),
	...
	var unmarshalled deneb.BeaconBlockHeader
