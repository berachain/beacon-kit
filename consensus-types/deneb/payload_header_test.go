...
func generateExecutionPayloadHeader() *deneb.ExecutionPayloadHeader {
...
func TestExecutionPayloadHeader_MarshalUnmarshalJSON(t *testing.T) {
	...
	var header deneb.ExecutionPayloadHeader
...
func TestExecutionPayloadHeader_Serialization(t *testing.T) {
	...
	var unmarshalled = &deneb.ExecutionPayloadHeader{}
...
func TestExecutionPayloadHeader_MarshalSSZTo(t *testing.T) {
	...
	testcases := []struct {
		name     string
		malleate func() *deneb.ExecutionPayloadHeader
		expErr   error
	}{
		{
			name:     "valid",
...
func TestExecutablePayloadHeaderDeneb_UnmarshalJSON_Error(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			var payload deneb.ExecutionPayloadHeader
...
func TestExecutionPayloadHeader_NewFromSSZ(t *testing.T) {
	...
	testCases := []struct {
		...
		expErr         error
		expectedHeader *deneb.ExecutionPayloadHeader
	}{
		{
			name: "Valid SSZ data",
			data: func() []byte {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.name == "Different fork version" {
				require.Panics(t, func() {
					_, _ = new(deneb.ExecutionPayloadHeader).
						NewFromSSZ(tc.data, tc.forkVersion)
				}, "Expected panic for different fork version")
			} else {
				header, err := new(deneb.ExecutionPayloadHeader).
					NewFromSSZ(tc.data, tc.forkVersion)
				if tc.expErr != nil {
...
func TestExecutionPayloadHeader_NewFromJSON(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			header, err := new(deneb.ExecutionPayloadHeader).NewFromJSON(
				tc.data,
				version.Deneb1(),
			)
