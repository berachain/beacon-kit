...
func TestNewValidatorFromDeposit(t *testing.T) {
	...
	tests := []struct {
		...
		want                      *deneb.Validator
	}{
		{
			name:   "normal case",
			pubkey: [48]byte{0x01},
			withdrawalCredentials: deneb.
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			got := deneb.NewValidatorFromDeposit(
				tt.pubkey,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.invalidSSZ {
				...
				var unmarshaled deneb.Validator
...
func TestValidator_GetWithdrawalCredentials(t *testing.T) {
	...
	tests := []struct {
		name      string
		validator *deneb.Validator
		want      deneb.WithdrawalCredentials
	}{
		{
			...
			want: deneb.NewCredentialsFromExecutionAddress(
				common.ExecutionAddress{0x01},
			),
		},
		{
			...
			validator: &deneb.Validator{
				WithdrawalCredentials: deneb.WithdrawalCredentials{0x00},
			},
			want: deneb.WithdrawalCredentials{0x00},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
