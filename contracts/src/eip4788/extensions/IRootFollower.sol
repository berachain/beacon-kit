...
interface IRootFollower {
    ...
    /// @dev Gets the address of the coinbase for the given block number. The
    /// size of
    /// the BeaconRootsContract stores the coinbase for the last 8191 blocks.
    /// Querying
    /// a block number greater than the last 8191 blocks will return an error.
    /// This also
    /// implies that actions should be invoked within 8191 blocks of being
    /// proposed.
    /// Otherwise any intended actions that were supposed to occur will be
    /// missed as the
    /// coinbase for the given block will no longer be available from the beacon
    /// root
    /// contract.
    ...
    /// @dev Gets the next block to be rewarded. This returns the greater of
    /// current
    /// previously invoked block + 1, or current block number - 8191 as that is
    /// the
    ...
    /// @dev Resets the next actionable block number to _block, used when out of
    /// the beacon root
    /// buffer.
