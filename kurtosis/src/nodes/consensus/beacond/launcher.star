...
def modify_genesis_files_deposits(plan, validators, genesis_files, chain_id, chain_spec, stored_configs):
    ...
    # Store the complete genesis directory with POL operator keys
    genesis_with_storage_store = StoreSpec(
        src = "/tmp/config_genesis/.beacond",
        name = "genesis-with-storage",
    )
    ...
    result = plan.run_sh(
        ...
        store = [genesis_with_storage_store, stored_configs[num_validators]],
        description = "Running modify genesis with deposits and extracting complete storage",
    )
