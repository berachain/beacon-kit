...
def get_client(plan, client_type, evm_genesis_data, jwt_file, el_service_name, network_params):
    if client_type == execution_types.CLIENTS.reth:
        return reth.get(plan, evm_genesis_data, jwt_file, el_service_name, network_params)
