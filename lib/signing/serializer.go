...
func (s *serializer[I, O]) ToSdkTx(input I, gasLimit uint64) (sdk.Tx, error) {
	...
	// are using the standard ante handler stuff I don't think we actually need
	// to.
	...
	// Lastly, we inject the signed ethereum transaction as a message into the
	// Cosmos Tx.
...
func (s *serializer[I, O]) ToSdkTxBytes(
	...
	// Then we use the clientCtx.TxConfig.TxEncoder() to encode the Cosmos
	// transaction into bytes.
