...
func runProxy(cmd *cobra.Command, args []string) error {
	...
	// check if the primary address is empty
	// when we go into this codepath, a primary address must already exist in the db
	if primaryAddr == "" { // check to see if we can start from an existing state
		...
		// no primary address specified, and none found -> must error
		if primaryAddress == "" {
	...
	// trust level is the fraction of the trust level
...
// checkForExistingProviders checks the db for existing primary and witness providers
