...
// DefineSSZ implements the ssz.Staticdect interface.
func (d *ExecutionPayloadHeader) DefineSSZ(c *ssz.Codec) {
	// Define the static data (fields and dynamic offsets)
	ssz.DefineStaticBytes(c, &d.ParentHash)           // Field  ( 0) -       ParentHash -  32 bytes
	ssz.DefineStaticBytes(c, &d.FeeRecipient)         // Field  ( 1) -     FeeRecipient -  20 bytes
	ssz.DefineStaticBytes(c, &d.StateRoot)            // Field  ( 2) -        StateRoot -  32 bytes
	ssz.DefineStaticBytes(c, &d.ReceiptsRoot)         // Field  ( 3) -     ReceiptsRoot -  32 bytes
	ssz.DefineStaticBytes(c, &d.LogsBloom)            // Field  ( 4) -        LogsBloom - 256 bytes
	ssz.DefineStaticBytes(c, &d.Random)               // Field  ( 5) -       PrevRandao -  32 bytes
	ssz.DefineUint64(c, &d.Number)                    // Field  ( 6) -      BlockNumber -   8 bytes
	ssz.DefineUint64(c, &d.GasLimit)                  // Field  ( 7) -         GasLimit -   8 bytes
	ssz.DefineUint64(c, &d.GasUsed)                   // Field  ( 8) -          GasUsed -   8 bytes
	ssz.DefineUint64(c, &d.Timestamp)                 // Field  ( 9) -        Timestamp -   8 bytes
	ssz.DefineDynamicBytesOffset(c, &d.ExtraData, 32) // Offset (10) -        ExtraData -   4 bytes
	ssz.DefineStaticBytes(c, &d.BaseFeePerGas)        // Field  (11) -    BaseFeePerGas -  32 bytes
	ssz.DefineStaticBytes(c, &d.BlockHash)            // Field  (12) -        BlockHash -  32 bytes
	ssz.DefineStaticBytes(c, &d.TransactionsRoot)     // Field  (13) - TransactionsRoot -  32 bytes
	ssz.DefineStaticBytes(c, &d.WithdrawalsRoot)      // Field  (14) -   WithdrawalRoot -  32 bytes
	ssz.DefineUint64(c, &d.BlobGasUsed)               // Field  (15) -      BlobGasUsed -   8 bytes
	ssz.DefineUint64(c, &d.ExcessBlobGas)             // Field  (16) -    ExcessBlobGas -   8 bytes

	// Define the dynamic data (fields)
	ssz.DefineDynamicBytesContent(c, &d.ExtraData, 32) // Field  (10) -        ExtraData - ? bytes
}
