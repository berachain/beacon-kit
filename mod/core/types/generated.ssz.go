...
// Hash: 988f3bcf26d72ffc51916e726b4102d8ba7952a23ed93e46f72ed0e2013fbe75
// Version: 0.1.3
package types
...
func (b *BeaconBlockBodyDeneb) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	{
		offset = 4 * len(b.BlobKzgCommitments)
		for ii := 0; ii < len(b.BlobKzgCommitments); ii++ {
			dst = ssz.WriteOffset(dst, offset)
			offset += len(b.BlobKzgCommitments[ii])
		}
	}
	for ii := 0; ii < len(b.BlobKzgCommitments); ii++ {
...
func (b *BeaconBlockBodyDeneb) UnmarshalSSZ(buf []byte) error {
	...
	// Field (4) 'BlobKzgCommitments'
	{
		...
		err = ssz.UnmarshalDynamic(buf, num, func(indx int, buf []byte) (err error) {
			copy(b.BlobKzgCommitments[indx][:], buf)
			return nil
		})
		if err != nil {
			return err
...
func (b *BeaconBlockBodyDeneb) SizeSSZ() (size int) {
	...
	for ii := 0; ii < len(b.BlobKzgCommitments); ii++ {
		size += 4
		size += len(b.BlobKzgCommitments[ii])
	}
