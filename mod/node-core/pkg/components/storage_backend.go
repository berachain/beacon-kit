...
// StorageBackendInput is the input for the storage backend for the depinject
// framework.
type StorageBackendInput struct {
	...
	PayloadCodec      encoding.SSZInterfaceCodec[*types.ExecutionPayloadHeader]
}

// ProvideStorageBackend provides a storage backend for the depinject framework.
	...
	return storage.NewBackend[
		*dastore.Store[types.BeaconBlockBody],
		...
		beacondb.New[
			...
			*types.Eth1Data,
			*types.Validator,
		](in.Environment.KVStoreService, &in.PayloadCodec),
		in.DepositStore,
	)
}
