...
func TestBlob_UnmarshalJSON(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			input: []byte(`"0x` + hex.EncodeToString(make([]byte, 131072)) + `"`),
			expected: []byte{0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
				0x0, 0x0, 0x0, 0x0, 0x0},
			wantErr: false,
		},
		{
...
func TestBlob_MarshalText(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			input: func() eip4844.Blob {
				var b eip4844.Blob
				copy(b[:], []byte{0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
		...
		},
		{
			...
			input: func() eip4844.Blob {
				var b eip4844.Blob
				copy(b[:], []byte{0x00, 0xFF, 0xAA, 0x55, 0x11, 0x22, 0x33, 0x44,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.NoError(t, err, "Test case: %s", tt.name)
			require.Equal(t, tt.expected, string(output), "Test case: %s", tt.name)
		})
	}
}
