...
func TestPaths(t *testing.T) {
	...
	bs := schema.DefineContainer(
		...
		schema.NewField("Validators", schema.DefineList(val, 1099511627776)),
	...
	typ, gindex, offset, err := val0PubKeyPath.GetGeneralizedIndex(blockHeader)
	require.NoError(t, err)
	require.Equal(t, schema.B48(), typ)
...
func TestEx(t *testing.T) {
	v := schema.DefineVector(schema.U64(), 4)

	objPath := merkle.ObjectPath[merkle.GeneralizedIndex, [32]byte](
		"5",
	)
	typ, gIndex, offset, err := objPath.GetGeneralizedIndex(v)
	require.NoError(t, err)
	require.Equal(t, schema.U64(), typ)
