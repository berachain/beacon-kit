...
// NoopGossipHandler2 is a gossip handler that simply returns the
// ssz marshalled data as a "reference" to the object it receives.
type NoopGossipHandler2[
	DataT constraints.SSZMarshallable, BytesT ~[]byte,
] struct{}

// Publish creates a new NoopGossipHandler2.
func (n NoopGossipHandler2[DataT, BytesT]) Publish(
	_ context.Context,
	data DataT,
) (BytesT, error) {
...
func (n NoopGossipHandler2[DataT, BytesT]) Request(
	_ context.Context,
	ref BytesT,
) (DataT, error) {
	var (
