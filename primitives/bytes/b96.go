...
// MarshalSSZTo ssz marshals the B96 object to a target array.
func (h B96) MarshalSSZTo(buf []byte) ([]byte, error) {
	dst := buf
	dst = append(dst, h[:]...)
	return dst, nil
}

// UnmarshalSSZ ssz unmarshals the B96 object.
func (h *B96) UnmarshalSSZ(buf []byte) error {
	if len(buf) != B96Size {
...
// SizeSSZ returns the ssz encoded size in bytes for the B96 object.
func (h B96) SizeSSZ() int {
	return B96Size
}

// HashTreeRootWith ssz hashes the B96 object with a hasher.
func (h B96) HashTreeRootWith(hh fastssz.HashWalker) error {
	hh.PutBytes(h[:])
	return nil
}

// GetTree ssz hashes the B96 object.
func (h *B96) GetTree() (*fastssz.Node, error) {
