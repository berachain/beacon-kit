...
func NewAnteHandler(
	...
	// Return a new Provider with the set up EVM and provided Cosmos ante
	// handlers
	return &Provider{
...
// AnteHandler returns a function that handles ante for both EVM and Cosmos
// transactions.
...
func (ah *Provider) AnteHandler() func(
	...
	return func(ctx sdk.Context, tx sdk.Tx, simulate bool) (sdk.Context, error) {
		// // If the transaction contains a single EVM transaction, use the EVM
		// ante handler
		// if len(tx.GetMsgs()) == 1 { //nolint:nestif // todo:fix.
		// 	if _, ok := tx.GetMsgs()[0].(*beacontypes.BeaconKitBlockContainer);
		// ok {
		// 		fmt.Println("ANTE HANDLER RUNNING??")
