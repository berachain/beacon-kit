...
- Once a validator is marked as active, `CometBFT` consensus will reach out to it for block proposals, validations and voting. The higher a validator's `EffectiveBalance`, the higher its voting power and the more frequently it is polled for block proposals.
