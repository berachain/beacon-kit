...
func (sp *StateProcessor) processWithdrawals(
	...
	expectedWithdrawals, err := st.ExpectedWithdrawals(blk.GetTimestamp().Unwrap())
	if err != nil {
		return err
	}
	...
	if !payloadWithdrawals[0].Equals(st.EVMInflationWithdrawal(blk.GetTimestamp().Unwrap())) {
